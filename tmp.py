...
def create_params():
    #return [rand() * 0.6 + 0.2, rand() * 0.6, rand() * 0.4 + 0.2, rand() * 0.4,
     #       rand() * 0.2 + 0.2, rand() + 0.2, rand(), rand()]
...
class Program4(Module):
    def forward(self, sample):
        if (1.0 >= self.thetas[3]):
            l_4_high = self.thetas[3]
        else:
            l_4_high = 1.0
        if (0.0 >= l_4_high):
        ...
        return ((l_1_cond * (1.0 if (sample == []) else 0.0))
                + ((1.0 - l_1_cond)
                   * ((l_6_cond * (0.0 if (sample == []) else
                                   ((density_IRNormal((((sample)[0] - self.thetas[0]) / self.thetas[1])))
                                    * (0.0 if ((sample)[1:] == [])
                                       else ((density_IRNormal(
                                               ((((sample)[1:])[0] - self.thetas[1]) / self.thetas[0])))
                                             * self.forward(((sample)[1:])[1:]))))))
                      + ((1.0 - l_6_cond) * (0.0 if (sample == []) else ((density_IRNormal(
                                    (((sample)[0] - self.thetas[2]) / self.thetas[3]))) * self.forward(
                                    (sample)[1:]))))
                      )))


    def generate(self):
        if (rand() >= self.thetas[3]):
            return []
        else:
            if (rand() >= self.thetas[0]):
                return [((randn() * self.thetas[0]) + self.thetas[1])] + [
                    ((randn() * self.thetas[1]) + self.thetas[0])] + self.generate()
            else:
                return [((randn() * self.thetas[2]) + self.thetas[3])] + self.generate()
...
class Main_flattening(Module):
    def forward(self, batch, indices, flattened_list):
        ...
        return (((torch.ones(batch).index_reduce_(0, tensor(indices[1]).to(torch.int64),
                 (l_6_cond * ((density_IRNormal(((tensor(flattened_list[1])[::2] - self.thetas[0]) / self.thetas[1])))
                         * (density_IRNormal(((tensor(flattened_list[1])[1::2] - self.thetas[1]) / self.thetas[0]))))),
                 'prod') * (1.0 - l_1_cond) * l_1_cond)
                * (torch.ones(batch).index_reduce_(0, tensor(indices[3]).to(torch.int64),
                                                   torch.zeros(len(indices[3])), 'amin')))
                + (torch.ones(batch).index_reduce_(0, tensor(indices[0]).to(torch.int64),
                    ((1.0 - l_1_cond) * (1.0 - l_6_cond) * (density_IRNormal(
                        ((tensor(flattened_list[0]) - self.thetas[2]) / self.thetas[3])))),
                    'prod') * l_1_cond))
...
def exp_convergence():
    ...
    guesses2, execution2, llh2 = converge_success_flattening(samples, search_params, 10, 10)

    print("n1: ", len(execution1))
    print("n2: ", len(execution2))
    print("t: ", execution1[len(execution1) - 1] / execution2[len(execution2) - 1])
    plt.plot(execution1, guesses1[:, 0], color="orange", label="Third program", linestyle="--")
    plt.plot(execution1, guesses1[:, 0], color="orange", marker="o")
    plt.plot(execution2, guesses2[:, 0], color="orange", label="Flattening-parallelized third program")
    ...
    plt.ylabel(r"Learning parameter ($\theta_i$)")
    plt.legend()
    #plt.xlim([0, 50])
    #plt.ylim([0, 1])
...
def exp_llh_comparison():
    ...
    print("t: ", execution1[len(execution1) - 1] / execution2[len(execution2) - 1])
    print("n1L: ", len(execution1))
    print("n2L: ", len(execution2))
