...
def minibatch_converge_success(i):
    ...
    """print("sample size: ", len(batches_of_samples))
    for batch in batches_of_samples:
        print("batch size, sample size: ", len(batch), len(batch[0]))"""
    ...
    for epoch in range(10):
        likelihoods = 0
        flat_list = [sample for sublist in batches_of_samples for sample in sublist]
        minibatch = math.ceil(len(flat_list)/batch_size)
        for segment in range(minibatch):
            likelihood = -torch.log(gausslist(flat_list[segment * batch_size: len(flat_list) if segment==minibatch-1 else segment * batch_size + batch_size]))
...
class Main(Module):
    def forward(self, batch):
        ...
        if (len(batch[0])==len(batch[len(batch)-1])):
            result *= torch.prod(((1.0 - l_1_cond) * ((density_IRNormal((torch.tensor(batch)[:,:] - self.thetas[2]) / self.thetas[1]) / self.thetas[1]))), dim=1)
        else:
            for i in range(len(result)):
                if (batch[i] == []):
                    continue
                else:
                    result[i] *= torch.prod(((1.0 - l_1_cond) * ((density_IRNormal((torch.tensor(batch[i])[:] - self.thetas[2]) / self.thetas[1]) / self.thetas[1]))), dim=0)
...
def exp_minibatch():
    ...
    for i in range(10):
        ...
        guesses, sample_params = minibatch_converge_success(i)
        end_time2 = time.time()
...
def exp_minibatch_convergence():
    ...
    plt.ylabel("Loss")
    plt.legend()
    plt.savefig('exp_minibatch_conv.png')
    plt.show()


#exp_convergence()
#exp_minibatch_convergence()
exp_minibatch()
