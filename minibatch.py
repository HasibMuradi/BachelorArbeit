...
def minibatch_converge_success(i):
    ...
    for epoch in range(10):
        likelihoods = 0
        for batch in batches_of_samples:
            segments = math.ceil(len(batch)/batch_size)
            if segments == 0:
                ...
                for segment in range(segments):
                    likelihood = -torch.log(gausslist(batch[segment*batch_size:(
                                    len(batch) if (segment == segments-1) else segment*batch_size+batch_size)]))
                    likelihood = torch.sum(likelihood)
                    likelihoods += likelihood.item()
                    likelihood.backward(retain_graph=True)
...
def exp_minibatch_performance():
    ...
    plt.legend()
    plt.savefig('exp_minibatch_performance.png')
    plt.show()
