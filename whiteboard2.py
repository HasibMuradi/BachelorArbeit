...
class Main_vertical(Module):
    def forward(self, index, list):
        ...
        for i in range(len(index)):
            result.index_put_((torch.tensor(index[i], dtype=torch.int64),),
                              ((1.0 - l_1_cond) * (density_IRNormal((torch.tensor(list[i]) - self.thetas[2]) / self.thetas[1]) / self.thetas[1]))
                          * result.index_select(0, torch.tensor(index[i], dtype=torch.int64)))
...
def converge_success_flattening(i):
    ...
    for epoch in range(10):
        start_time = time.time()
        for i in range(int(len(samples)/100)):
            likelihood = -torch.log(gausslist(samples[100*i: 100*i+100], indices_lists[i], flattened_lists[i]))
            likelihood = torch.sum(likelihood)
            likelihood.backward(retain_graph=True)
            optimizer.step()
            optimizer.zero_grad()
...
def exp_performance():
    ...
    #plt.savefig('without_preprocessing.png')
    plt.show()
